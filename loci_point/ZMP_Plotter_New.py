...
def CSV_reader_df_maker_General(csv_name, csv_folder_name, object_name, param_name_list, header_list, number_or_name,
    ...
    # column_list_o = list(filter(lambda local_x: local_x.endswith(object_name), df.columns))
    # column_list_o = list(filter(lambda local_x: local_x.endswith(object_name), df.columns))
    # # column_list_o = list(filter(lambda local_x: local_x.contains(object_name), df.columns))

    # column_list_o = list(filter(lambda local_x: local_x.find(object_name), df.columns))
    column_list_o = list(filter(lambda local_x: object_name in local_x, df.columns))
    # column_list_o = list(filter(df.columns.str.contains(object_name), df.columns))
...
def ZMP_Plotter(df_plt, save_o_TOV, p_show_TOV, fig_name, is_W, max_time):
    """
            Function returns dataframe with mapped values.
        """
    ...
    ZMP_header_list = ['Time', 'Phase A-bus(j)', 'Phase B-bus(j)', 'Phase C-bus(j)', 'Phase A-bus(i)', 'Phase B-bus(i)',
                       'Phase C-bus(i)']
    ZMP_header_list = constants.Digsilent.ZMP_header_list_New

    # if is_W:
    #     y_a = df_plt.loc[:, constants.Digsilent.ZMP_header_list[1]]
    # else:
    #     y_a = df_plt.loc[:, constants.Digsilent.ZMP_header_list[4]]
    #
    # if is_W:
    #     y_b = df_plt.loc[:, constants.Digsilent.ZMP_header_list[2]]
    # else:
    #     y_b = df_plt.loc[:, constants.Digsilent.ZMP_header_list[5]]
    #
    # if is_W:
    #     y_c = df_plt.loc[:, constants.Digsilent.ZMP_header_list[3]]
    # else:
    #     y_c = df_plt.loc[:, constants.Digsilent.ZMP_header_list[6]]

    if is_W:
        y_a = df_plt.loc[:, ZMP_header_list[1]]
    else:
        y_a = df_plt.loc[:, ZMP_header_list[1]]

    if is_W:
        y_b = df_plt.loc[:, ZMP_header_list[2]]
    else:
        y_b = df_plt.loc[:, ZMP_header_list[2]]

    if is_W:
        y_c = df_plt.loc[:, ZMP_header_list[3]]
    else:
        y_c = df_plt.loc[:, ZMP_header_list[3]]
    ...
    if is_W:
        ...
        # plt.axis(xmin=int(1975), xmax=(2000), ymin=int(-8), ymax=int(8))
        # plt.axis(xmin=int(1975), xmax=int(2000), ymin=int(-8), ymax=int(8))
        # plt.axis(xmin=int(0), xmax=(2000), ymin=int(-8), ymax=int(8))
        plt.axis(xmin=int(0), xmax=(1000), ymin=int(-8), ymax=int(8))
        #plt.axis(xmin=int(0), xmax=(400), ymin=int(-8), ymax=int(8))
    else:
    ...
    plt.legend(constants.plotting.ZMP_legend)
    ...
    if is_W:
        plot_name = fig_name + '_Wo' + constants.plotting.figure_type
    else:
        plot_name = fig_name + '_Du' + constants.plotting.figure_type
    ...
    fig_path = common.get_local_file_path_withfolder(file_name=plot_name,
                                                     folder_name=constants.plotting.ZMP_plots_folder)
    ...
    #     plot_name = fig_name + '_j_Bus_Total' + constants.plotting.figure_type
    #     fig_path = common.get_local_file_path_withfolder(file_name=plot_name,
    ...
    # else:
    #     plt.axis(xmin=int(0), xmax=int(max_time), ymin=int(-600), ymax=int(600))
    #     plot_name = fig_name + '_i_Bus_Total' + constants.plotting.figure_type
    #     fig_path = common.get_local_file_path_withfolder(file_name=plot_name,
    #                                                      folder_name=constants.plotting.TOV_plots_folder)
    #     if save_o_TOV:
    #         plt.savefig(fig_path)
...
def Z_crossing_finder(df):
    ...
    b = df['Phase B'].tolist()
    c = df['Phase C'].tolist()
    zero_crossings_a = np.where(np.diff(np.sign(a)))[0]
    zero_crossings_b = np.where(np.diff(np.sign(b)))[0]
    zero_crossings_c = np.where(np.diff(np.sign(c)))[0]

    if len(zero_crossings_a) > 0:
        Z_time_a = df.loc[zero_crossings_a[1] + 1, 'Time'] - df.loc[zero_crossings_a[0] + 1, 'Time']
    else:
        Z_time_a = -1000

    if len(zero_crossings_b) > 0:
        Z_time_b = df.loc[zero_crossings_b[1] + 1, 'Time'] - df.loc[zero_crossings_b[0] + 1, 'Time']
    else:
        Z_time_b = -1000

    if len(zero_crossings_c) > 0:
        Z_time_c = df.loc[zero_crossings_c[1] + 1, 'Time'] - df.loc[zero_crossings_c[0] + 1, 'Time']
    else:
        Z_time_c = -1000
        # Z_time_b = df.loc[zero_crossings_b[1] + 1, 'Time'] - df.loc[zero_crossings_b[0] + 1, 'Time']
        # Z_time_c = df.loc[zero_crossings_c[1] + 1, 'Time'] - df.loc[zero_crossings_c[0] + 1, 'Time']

    return Z_time_a, Z_time_b, Z_time_c
...
def TOV_SC_Runner(TOV_List, object_Name, output_name, is_pow, TOV_Limit, plot_TOV, save_o_TOV, p_show_TOV,
    ...
    """
    if first_col_in:
        TOV_header_List = constants.Digsilent.ZMP_header_list_New
    else:
        TOV_header_List = constants.Digsilent.ZMP_header_list_no_New_fc

    TOV_POW_List = np.arange(0, 360 + 18, 18)

    if is_pow:
        final_Df_W = pd.DataFrame(TOV_POW_List, columns=['POW'])
        final_Df_D = pd.DataFrame(TOV_POW_List, columns=['POW'])
        final_Df = pd.DataFrame(TOV_POW_List, columns=['POW'])
        csv_List = ['{}{}'.format(x, '.csv') for x in TOV_POW_List]
        results_folder = constants.folder_file_names.ZMP_folder

    else:
        final_Df_W = pd.DataFrame(TOV_List, columns=['Scenario'])
        final_Df_D = pd.DataFrame(TOV_List, columns=['Scenario'])
    ...
    final_Df_W['A_Z_Crossing'] = np.nan
    final_Df_W['B_Z_Crossing'] = np.nan
    final_Df_W['C_Z_Crossing'] = np.nan
    final_Df_D['A_Z_Crossing'] = np.nan
    final_Df_D['B_Z_Crossing'] = np.nan
    final_Df_D['C_Z_Crossing'] = np.nan

    final_Df['A_Z_Crossing_W'] = np.nan
    final_Df['B_Z_Crossing_W'] = np.nan
    final_Df['C_Z_Crossing_W'] = np.nan
    final_Df['A_Z_Crossing_D'] = np.nan
    final_Df['B_Z_Crossing_D'] = np.nan
    final_Df['C_Z_Crossing_D'] = np.nan
    ...
    for i in range(0, len(csv_List)):
        for j in range(0, len(object_Name)):
            ...
            if plot_TOV:
                # is_W = True
                ZMP_Plotter(scan_df, save_o_TOV, p_show_TOV, TOV_List[i], is_W, max_time)
                # is_W = False
                # ZMP_Plotter(scan_df, save_o_TOV, p_show_TOV, TOV_List[i], is_W, max_time)
            Z_a, Z_b, Z_c = Z_crossing_finder(scan_df)

            if is_W:
                final_Df_W.loc[i, 'A_Z_Crossing'] = Z_a
                final_Df_W.loc[i, 'B_Z_Crossing'] = Z_b
                final_Df_W.loc[i, 'C_Z_Crossing'] = Z_c
                final_Df.loc[i, 'A_Z_Crossing_W'] = Z_a
                final_Df.loc[i, 'B_Z_Crossing_W'] = Z_b
                final_Df.loc[i, 'C_Z_Crossing_W'] = Z_c
            else:
                final_Df_D.loc[i, 'A_Z_Crossing'] = Z_a
                final_Df_D.loc[i, 'B_Z_Crossing'] = Z_b
                final_Df_D.loc[i, 'C_Z_Crossing'] = Z_c
                final_Df.loc[i, 'A_Z_Crossing_D'] = Z_a
                final_Df.loc[i, 'B_Z_Crossing_D'] = Z_b
                final_Df.loc[i, 'C_Z_Crossing_D'] = Z_c

            # final_Df_D['A_Z_Crossing'] = np.nan
            # final_Df_D['B_Z_Crossing'] = np.nan
            # final_Df_D['C_Z_Crossing'] = np.nan

        # scan_df_2 = CSV_reader_df_maker_General(csv_name=csv_List[i],
        #                                         csv_folder_name=results_folder,
        ...
        # scan_df = scan_df_1.loc[:, TOV_header_List[1:]] + scan_df_2.loc[:, TOV_header_List[1:]]
        # if first_col_in:
        #     scan_df[TOV_header_List[0]] = np.nan
        #     scan_df[TOV_header_List[0]] = scan_df_1.loc[:, TOV_header_List[0]]
        #
        # if is_pow:
        #     POW = i * 18
        # else:
        ...
        # if plot_TOV:
        #     is_j = True
        #     ZMP_Plotter(scan_df, save_o_TOV, p_show_TOV, TOV_List[i], is_j, max_time)
        #     is_j = False
        #     ZMP_Plotter(scan_df, save_o_TOV, p_show_TOV, TOV_List[i], is_j, max_time)
    ...
    #
    FILE_PTH_OUTPUT_1 = common.get_local_file_path_withfolder(file_name=output_TOV_list[0], folder_name='ZMP_Output')
    #
...
if __name__ == '__main__':
    ...
    object_Name_TOV = 'Woodland - Dunstown 400 kV Cbl2'
    object_Name_TOV = 'Woodland - Dunstown'
    object_Name_TOV = ['Woodland - Dunstown 400 kV Cbl2', 'Woodland - Dunstown 400 kV Cbl1']
    # object_Name_TOV = ['WOO\400 kV Dunstown DB(1)', 'DSN\400 kV Woodland DB']
    object_Name_TOV = ['Dunstown DB(1)', 'Woodland DB']
    first_col_in = True
    # param_num_list = [1, 2, 3, 4, 5, 6]
    ...
    TOV_List = ['W_PRF_A_0']
    # TOV_List = ['W_PRF_B_0'],
    TOV_List = ['W_PRF_B_0', 'W_PRF_A_0']
    TOV_List = ['W_PRF_B_0', 'W_PRF_A_0', 'W_PRF_C_0', 'D_PRF_B_0', 'D_PRF_A_0', 'D_PRF_C_0', 'D_PRF_A_Peak',
                'W_PRF_A_Peak']
    TOV_List = ['W_PRF_A_S1', 'W_PRF_A_S2', 'W_PRF_POW']
    TOV_List = ['W_PRF_A_0']
    TOV_List = ['W_PRF_B_0', 'W_PRF_A_0', 'W_PRF_C_0', 'D_PRF_B_0', 'D_PRF_A_0', 'D_PRF_C_0', 'D_PRF_A_Peak',
                'W_PRF_A_Peak','W_PRF_A_S1', 'W_PRF_A_S2', 'W_PRF_POW','D_PRF_A_S1', 'D_PRF_A_S2', 'D_PRF_POW']

    TOV_List = ['W_PRF_A_ACR_0', 'W_PRF_A_ACR_10', 'W_PRF_A_ACR_20','W_PRF_A_ACR_30','W_PRF_A_ACR_40','W_PRF_A_ACR_50']
    TOV_List = ['D_PRF_POW']
    TOV_List = ['W_PRF_A_ACR_0', 'W_PRF_A_ACR_10', 'W_PRF_A_ACR_20', 'W_PRF_A_ACR_30', 'W_PRF_A_ACR_40','W_PRF_A_ACR_50',\
                'W_PRF_A_ACR_60','W_PRF_A_ACR_70','W_PRF_A_ACR_80','W_PRF_A_ACR_90','W_PRF_A_ACR_100','W_PRF_A_ACR_110']

    TOV_List = ['D_PRF_A_ACR_0', 'D_PRF_A_ACR_10', 'D_PRF_A_ACR_20', 'D_PRF_A_ACR_30', 'D_PRF_A_ACR_40',
                'D_PRF_A_ACR_50', \
                'D_PRF_A_ACR_60', 'D_PRF_A_ACR_70', 'D_PRF_A_ACR_80', 'D_PRF_A_ACR_90', 'D_PRF_A_ACR_100',
                'D_PRF_A_ACR_110']
    ...
    output_name = ['W_PRF_A_0_Results']
    output_name = ['ZMP_All_Results']
    output_name = ['ZMP_Solution1_Results']
    output_name = ['ZMP_Results_New']
    output_name = ['ZMP_Results_W_ACR']
    output_name = ['D_PRF_POW_Results']
    output_name = ['ZMP_W_ACR_Results_New']
    output_name = ['ZMP_D_ACR_Results_New']
